"""
Milk Production sum_losses Convergence Study — 2×2 factorial (active_g × sum_losses)

Evaluates the effect of `sum_losses` (and its interaction with `active_g`) on
convergence of the milk production dataset (156 monthly observations, 1962-1974)
using a 6-stack Generic architecture with 100 independent random-seed runs per
configuration.

When `sum_losses=True`, the training loss adds a weighted backcast reconstruction
term (0.25 × loss(backcast, zeros)) that pushes backcasts to fully reconstruct
the input signal.  This study measures whether that regularisation improves
convergence stability and final forecast accuracy.

Modeled after Part 8 of run_experiments.py and the milk convergence study, adapted
for the single-series TimeSeriesDataModule workflow.
...
Experimental Conditions (2×2 factorial):
  ...
  - 100 runs per config (400 total), each with a unique random seed
...
Results saved to experiments/results/milk_sumloss_convergence/

Usage:
    python experiments/run_milk_sumloss_convergence.py
    python experiments/run_milk_sumloss_convergence.py --max-epochs 500 --n-runs 100
    python experiments/run_milk_sumloss_convergence.py --config MilkSL_sumLosses --max-workers 8
"""
...
# Experimental conditions — 2×2 factorial: active_g × sum_losses
MILK_SUMLOSS_CONFIGS = {
    "MilkSL_baseline":      {"active_g": False, "sum_losses": False, "activation": "ReLU"},
    "MilkSL_activeG":       {"active_g": True,  "sum_losses": False, "activation": "ReLU"},
    "MilkSL_sumLosses":     {"active_g": False, "sum_losses": True,  "activation": "ReLU"},
    "MilkSL_activeG+sumL":  {"active_g": True,  "sum_losses": True,  "activation": "ReLU"},
}

# Paths
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results", "milk_sumloss_convergence")
MILK_CSV_PATH = os.path.join(
    os.path.dirname(__file__), "..", "src", "lightningnbeats", "data", "milk.csv"
)

# CSV schema — extends milk convergence study with sum_losses column
MILK_SUMLOSS_CSV_COLUMNS = [
...
def init_csv(path):
    ...
    if not os.path.exists(path):
        with open(path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=MILK_SUMLOSS_CSV_COLUMNS)
            writer.writeheader()
...
def run_single_milk_sumloss_experiment(
    config_name, active_g, sum_losses, activation, run_idx, seed,
    ...
    result = {
        "experiment": "milk_sumloss_convergence",
...
def run_milk_sumloss_study(
    ...
    """Run the full milk sum_losses convergence study with parallel execution.
    ...
    csv_path = os.path.join(RESULTS_DIR, "milk_sumloss_convergence_results.csv")
    init_csv(csv_path)
    ...
    total_jobs = len(jobs)
    if total_jobs == 0:
        print("All milk sum_losses convergence study runs already complete.")
    else:
        print(f"\nMilk sum_losses Convergence Study — {total_jobs} jobs, "
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for job in jobs:
                future = executor.submit(
                    run_single_milk_sumloss_experiment, **job
                )
...
def print_summary_statistics(csv_path):
    ...
    print("\n" + "=" * 80)
    print("MILK sum_losses CONVERGENCE STUDY — SUMMARY STATISTICS")
    print("=" * 80)
    ...
    print("=" * 80)
    header = (f"{'Config':<25} {'Conv%':>6} {'ValLoss':>12} "
    ...
    for config_name in sorted(df["config_name"].unique()):
        ...
        print(f"{config_name:<25} {conv_pct:>6} {val_str:>12} "
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Milk Production sum_losses Convergence Study — "
            "2×2 factorial (active_g × sum_losses) on small-scale N-BEATS"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=list(MILK_SUMLOSS_CONFIGS.keys()),

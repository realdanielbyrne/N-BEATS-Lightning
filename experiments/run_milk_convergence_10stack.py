"""
Milk Production Convergence Study (10-Stack) — active_g effect on N-BEATS

Evaluates whether `active_g` stabilizes convergence on the milk production dataset
(156 monthly observations, 1962-1974) using a 10-stack Generic architecture with
100 independent random-seed runs per configuration.

Modeled after run_milk_convergence.py (6-stack variant), scaled up to 10 stacks
to test whether the active_g stabilization effect strengthens with deeper models.
...
Experimental Conditions:
  ...
  - 10 stacks of Generic blocks, 1 block/stack, shared weights
...
Results saved to experiments/results/milk_convergence_10stack/

Usage:
    python experiments/run_milk_convergence_10stack.py
    python experiments/run_milk_convergence_10stack.py --max-epochs 500 --n-runs 100
    python experiments/run_milk_convergence_10stack.py --config Milk10_activeG --max-workers 8
"""
...
# Model hyperparameters (from simple_example.ipynb, scaled to 10 stacks)
FORECAST_LENGTH = 6
...
MILK_CONVERGENCE_CONFIGS = {
    "Milk10_baseline": {"active_g": False, "activation": "ReLU"},
    "Milk10_activeG":  {"active_g": True,  "activation": "ReLU"},
}

# Paths
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results", "milk_convergence_10stack")
...
def run_milk_convergence_study(
    ...
    csv_path = os.path.join(RESULTS_DIR, "milk_convergence_10stack_results.csv")
    init_csv(csv_path)
    ...
    if total_jobs == 0:
        ...
        print(f"\nMilk Convergence Study (10-Stack) — {total_jobs} jobs, "
...
def print_summary_statistics(csv_path):
    ...
    print("\n" + "=" * 75)
    print("MILK CONVERGENCE STUDY (10-STACK) — SUMMARY STATISTICS")
    print("=" * 75)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Milk Production Convergence Study (10-Stack) — "
            "active_g effect on N-BEATS"
        ),
    )
    parser.add_argument(
